...
mkdir ${TEST_DIR}/docroot/profiles/thunder-distribution
shopt -s extglob
rsync -a . ${TEST_DIR}/docroot/profiles/thunder-distribution --exclude docroot

drush make -y --no-core ${TEST_DIR}/docroot/profiles/thunder-distribution/drupal-org.make ${TEST_DIR}/docroot/profiles/thunder-distribution

# Get development branch of Thunder Admin theme (to use same admin theme as for composer build)
rm -rf ${TEST_DIR}/docroot/profiles/thunder-distribution/themes/thunder_admin
git clone --depth 1 --single-branch --branch ${THUNDER_ADMIN_BRANCH} https://github.com/BurdaMagazinOrg/theme-thunder-admin.git ${TEST_DIR}/docroot/profiles/thunder-distribution/themes/thunder_admin
