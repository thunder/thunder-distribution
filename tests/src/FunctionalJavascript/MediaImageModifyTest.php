...
class MediaImageModifyTest extends ThunderJavascriptTestBase {
  ...
  public function testRemoveAdd(): void {
    ...
    $this->assertEquals($file->getFilename(), $this->getSession()->evaluateScript('jQuery(\'[data-drupal-selector="edit-field-paragraphs-0-preview"] div.paragraph-preview__thumbnail img\').attr(\'src\').split(\'?\')[0].split(\'/\').splice(-1)'), 'Image file should be identical to previously selected.');
