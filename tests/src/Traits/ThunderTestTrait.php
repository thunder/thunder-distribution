...
trait ThunderTestTrait {
  ...
  /**
   * Get a meade from the database based on its name.
   *
   ...
   *
   * @param string|\Drupal\Component\Render\MarkupInterface $name
   *   A file name, usually generated by $this->randomMachineName().
   * @param bool $reset
   *   (optional) Whether to reset the entity cache.
   *
   * @return \Drupal\file\FileInterface|bool
   *   A file entity matching $name.
   *
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function getFileByName($name, $reset = FALSE) {
    if ($reset) {
    ...
    $files = \Drupal::entityTypeManager()
      ->getStorage('file')
      ->loadByProperties(['filename' => $name]);
    return reset($files);
  }
